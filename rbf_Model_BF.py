...
if(bloom is not None):
    size = float(size) +  (float)(bloom.num_bits_m)/8.0
print('Size of Model + BF (in Bytes) : ', size)
...
print('Predict FNs on Train set: ')
...
#print(num_neg)
# print('Still predicting on train: ')
print('Predict FPs on Train set: ')
print('False Positives: ', conf_matrix[0, 1])
orig_fp_rate = (float) (conf_matrix[0, 1]/float(num_neg))
print('False Positive Rate: ', orig_fp_rate)
print('Overall False Positive Rate: ', orig_fp_rate + ( 1 - orig_fp_rate)*fpr_b)
ov_fpr_train = orig_fp_rate + ( 1 - orig_fp_rate)*fpr_b
print(ov_fpr_train)
...
print('False Positive Rate: ', orig_fp_rate)
print('Overall False Positive Rate: ', orig_fp_rate + ( 1 - orig_fp_rate)*fpr_b)
ov_fpr_test = orig_fp_rate + ( 1 - orig_fp_rate)*fpr_b
print(ov_fpr_test)
FP =  orig_fp_rate + ( 1 - orig_fp_rate)*fpr_b
...
bloom = BloomFilter(max_elements=bloomSize, error_rate=err_rate)
...
for i in range(len(newTestX)):
    if(newTestX[i] in bloom):
        falsePositive = falsePositive + 1

# print(falsePositive, '    :    ', size)
n = float(falsePositive) / float(size)
filterSize = float(bloom.num_bits_m)/float(8)

print("Configured False Positive Rate : ", err_rate)
print("Traditional False Positive Rate : ", n)
# print("Trad BF Size in bytes : ", filterSize)

filename = 'trad_bloom' + str(np.random.randint(1000))
print(filename)
dump(bloom, filename + '.compressed', compress=True)
fName = filename + '.compressed'
size = os.stat(fName).st_size
# print('gggggggggg')
# print(type(size))
print('Trad Bloom Filter stored in: ', fName)
print('Size of Trad BF: ', size)
print()
# print('fileSize')

# print('Wanted FPR Traditional: ', err_rate)
# print('Actual FPR Traditional: ', n)
# print()
# print('Overall FPR TRAIN: ', ov_fpr_train)
# print('Overall FPR test: ', ov_fpr_test)
# print(err_rate, ',', n, ',', ov_fpr_train, ',', ov_fpr_test)
